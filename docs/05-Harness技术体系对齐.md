...
## 公众号 Agent 章节入口 · 2026-09-07

[配套文章](./AIBuilder第一周.md) · [Agent 三层图](./assets/learning-map/agent-three-layer-aigc.png) · [角色、Prompt 与测试对应表](./README.md#agent-章节怎样对照源码)。个人助理、多角色教学工作流、运行支撑是三个不同层次；不把内部函数分工写成自由多 Agent 已部署。

以下是注明日期的工程盘点，不是实时审计；历史工具数、测试数和缺口须按当前代码复核。本次仅同步阅读入口，没有重新执行生产与真实渠道验证。
