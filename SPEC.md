...
### ADR-060 · 过程型开源学习入口：文章、学习地图与开发工作台相互追溯

**Context**：第一周产出不能只展示功能与测试结果。需求、设计规范、页面、代码、Agent 与验收材料已存在，但旧文档入口停留在 V0.18/V0.19，读者难以区分历史方案与当前实现。

**Decision**：

1. 在现有文档体系增加一个静态学习地图，按产品问题 → 需求 → 设计 → 前后端 → Agent → 验证 → 版本迭代组织源码与可视化入口，不新增业务应用。
2. 开发工作台继续承担版本治理；用需求、实现和验收链接展示交付过程，不伪装成自动同步的项目管理后端。历史版本任务数与当前全量测试数分别标注。
3. Design Tokens 以现有页面实际 CSS 为实现依据；通用设计规范是目标合同，尚未完成全站 token 统一。真实页面截图、历史原型与解释性示意图分别标记。
4. 文章是导读，仓库保存可复用产物与依据。公开内容不包含私人学习实例、生产凭据或原始私人开发历史；源码 RC 与生产上线继续独立验收。

**Plan / Acceptance**：整理文档入口与历史标识；补学习地图及工作台过程入口；验证相对链接、390px/桌面布局与截图；重写文章并核对图文/仓库链接；在干净公开历史上仅同步本次文档产物。不得借此变更业务代码或宣称飞书/生产已验收。

---

### ADR-061 · 第一周交付以产品架构、需求合同和 AI Design 驱动文章

**Context**：用户否定以登录入口、局部工作台及单个提醒 API 代替完整交付的文章。已确认取消 3000 字硬限，先补齐可复用产物，再用真实体验和方法论写作。

**Decision / Plan**：本轮只修改文档、图示与文章包装，不重做业务功能。补当前 PRD（目标、对象、范围、流程、规则、验收与追踪），形成可供 AI 读取的视觉/交互规范；产品分层图与系统/Agent 架构图各司其职。截图只用实际系统内部页面或明确注明来源的本地演示，不以登录页、说明区或伪造运行数据冒充。完整开发工作台保留版本、任务依赖与验收页，文章末按产物类型提供链接。历史文档不删除，以日期和覆盖关系保留演进。

**Acceptance**：阅读者能够沿一个需求追到产品对象、设计、服务、测试和发布门；图中文本可读、图文互补；链接、桌面/390px、图片加载与复制检查通过；截图披露运行环境和数据性质。公开仓库只同步显式清单，不推私人历史，不宣称生产或飞书已验收。

**呈现补充**：用户要求架构总图以 AIGC 提升视觉表达。保留可编辑 HTML 内容源，以内置生图生成独立展示版；公开 prompt、参考来源、原生尺寸与语义校对说明，错误连线必须修正。内部页面采用用户提供的三张真实截图。文章练习提供无需真实模型调用的零额度守卫测试与实跑结果。

**读者修订计划**：开头先介绍产品与用户收益；以“每天 21 点提醒我学习”为贯穿示例，按需求、AI Design、前后端、Agent、验收说明输入文件、执行动作与可见结果。明确 OpenClaw 承载个人助理，飞书仅为 IM 收发通道，LearnBuddy 保存学习事实并独立调度提醒。AI Design 补齐 Token/Scale、阴影、产品图标、规则载入与 CSS 载入的区别；不虚构自动安装的 Skill。图源同步修改，AIGC 总图重绘；公众号正文显式给出完整仓库地址。不修改业务运行时，不把真实渠道方案写成已完成验收。

**配图终版**：按用户对第二、六张正文图的批注，AI Design 与 Agent 开发合同也改为内置 AIGC 展示图；仅替换这两张插图，保留原 HTML 内容源与原图，补充生成提示词和来源。正文维持 7 张图；新图须通过中文、关系连线、桌面/390px 和正文复制检查。

---

### ADR-062 · 公众号配套资料同步与地址兼容（2026-09-07）

**范围与计划**：优先同步作者确认的第一周最终稿、Agent 三层图和生成 Prompt，修正目录与学习地图。既有公众号路径及锚点不迁移、不删除；旧图保留供历史引用。ADR-061 的提醒贯穿与七图为历史轮次，当前以产品全流程、三层 Agent 和六图为准。不改业务代码或生产部署。

**页面合同与状态**：学习地图面向复刻项目的读者，主行动是定位架构、源码和测试；保留原导航及六个章节锚点。静态页面不写业务数据；图片提供替代文本及原图链接，手机自适应，链接键盘可达。已有、历史、待验收状态分别注明，不用图示制造运行成功结论。

**验证与发布**：核对最终稿规范化一致性、配图清单、内容哈希与公众号旧路径，检查学习地图桌面及 390px；公开发布只提交显式清单到已清理的远端历史，再核对远端 blob。私人历史、未跟踪预览、学习数据与凭据不纳入。真实渠道和生产门保持待验收。
